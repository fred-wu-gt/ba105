...
<% 
     ...
     List<Fav_shopVO> listAA = favShopSvc.findByMem_no(memVO.getMem_no());
...
<%-- 			<jsp:useBean id="ShopSvc" scope="page" class="com.shop.model.ShopService" /> --%>
			...
			<div class="col-xs-12 col-sm-12">
				<table class="table table-hover"  >
					...
					<c:forEach var="favshopVO" items="${listAA}">
						<tr>
...
<%-- 							<td width="100"  height=auto><img src="<%=request.getContextPath() %>/front-end/DBPicReader/DBPicReader.do?action=shop&id_no=${favshopVO.shop_no}" alt="暫無圖片"  ></td> --%>
							...
							<td width="100"  height=auto><a href='#shop_area${favshopVO.shop_no}' data-toggle="modal" > 
							<img src="<%=request.getContextPath() %>/front-end/DBPicReader/DBPicReader.do?action=shop&id_no=${favshopVO.shop_no}" alt="暫無圖片" ></a> 
			    ...
			    <div class="tab-content">
			        ...
			        <div role="tabpanel1" class="tab-pane " id="m_table2${favshopVO.shop_no}"><!-- 活動區塊開始  -->
							...
							<div class="modal-body">
							 ...
							 <c:forEach var="activityVO" items="${ actiSvc.findByShopno(favshopVO.shop_no) }">
							  <c:if test="${not empty activityVO}">
								<a href="<%=request.getContextPath()%>/front-end/activity/activity_mem_home.jsp">
								<img width="200"  src="data:image/jpeg;base64,${activityVO.act_pic_base64}"></a>
								</c:if>
								
											${activityVO.act_name}
							</c:forEach>
								<c:if test="${empty actiSvc.findByShopno(favshopVO.shop_no) }">
											<p>此商家無舉辦活動</p>
								</c:if>
					...
					<div role="tabpanel1" class="tab-pane " id="m_table3${favshopVO.shop_no}"><!-- 文章區塊開始  -->
							...
							<div class="modal-header">
								...
								<img width="200"  src="data:image/jpeg;base64,${wrilSvc.findByshopno(favshopVO.shop_no).wrt_photo_base64}">
								</a>
								</c:if>
